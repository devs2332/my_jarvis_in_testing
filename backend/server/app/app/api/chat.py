...
from backend.server.app.dependencies import DBSession, CurrentUser
from backend.server.app.schemas.chat import (
    ChatRequest, ChatResponse, ConversationListItem,
    MessageResponse,
)
from backend.server.app.services.chat_service import (
...
from backend.server.app.core.security import verify_access_token
from backend.server.app.database import async_session_factory
...
async def chat(request: ChatRequest, user: CurrentUser, db: DBSession):
    ...
    from backend.server.app.models.subscription import Subscription
    ...
    from backend.server.ai.llm_router import route_model
    from backend.server.ai.agent import run_agent
...
async def websocket_chat(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            # Stream response
            from backend.server.ai.llm_router import route_model
            from backend.server.ai.agent import stream_agent
